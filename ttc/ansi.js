 ...
 */
const fgColor = (n) => {
    if (!(0, funcs_1.isNumberInRange)(n)) {
        (0, funcs_1.logError)(`${n} is not correct foreground terminal color code`);
        return "\x1b[0m";
    }
    return `\x1b[38;5;${n}m`;
};
 ...
 */
const bgColor = (n) => {
    if (!(0, funcs_1.isNumberInRange)(n, 'bg')) {
        (0, funcs_1.logError)(`${n} is not correct background terminal color code`);
        return "\x1b[0m";
    }
    return `\x1b[48;5;${n}m`;
};
 ...
 * @returns string
 */
const fgColorRGB = (r, g, b) => {
    if ((0, funcs_1.isEigjtBit)(r) && (0, funcs_1.isEigjtBit)(g) && (0, funcs_1.isEigjtBit)(b)) {
 ...
 * @returns string
 */
const fgColorHex = (hex) => {
    const hexArr = (0, funcs_1.parseHex)(hex);
    if (hexArr.length === 0) {
        return `\x1b[0m`;
    }
